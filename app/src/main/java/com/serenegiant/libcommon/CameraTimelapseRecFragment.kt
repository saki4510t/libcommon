 ...
 */
class CameraTimelapseRecFragment : AbstractCameraFragment() {
...
//		sampler?.release()
	...
	private fun createRecorder(
		...
		val recorder = MediaAVTimelapseRecorder(
		...
		// XXX 動画のフレームレートはMediaAVTimelapseRecorderで決まるので
		//     動画エンコーダーへのフレームレート指定はなんでもかまわないけど
		//     VideoConfigから取得するようにした方(-1を指定)が無難かも
		// mVideoEncoder!!.setVideoConfig(-1, -1, -1)
		// XXX タイムラプス録画時には音声には対応していないので音声用エンコーダーは生成しない
//		if (audio_source >= 0) {
...
//		}
				...
				if (encoder is SurfaceEncoder) {
					if (mEncoderSurface == null) {
						...
						if (surface != null) {
							...
							try {
								// XXX ここでカメラからの映像フレームレートをVIDEO_FPS定数よりも遅くなるように制限するとタイムラプス動画になる
								// 	   VIDEO_FPS=30でここのmaxFps=5だと30/5=6でおよそ6倍速になる
								addSurface(surface, INPUT_FPS)
							} catch (e: Exception) {
								mEncoderSurface = null
								throw e
							}
	...
	companion object {
		...
		private val TAG = CameraTimelapseRecFragment::class.java.simpleName
		 ...
		 */
		private val INPUT_FPS = Fraction(5)
