	...
	private fun checkPermissionResult(
		...
		if (!result && permission != null) {
			if (Manifest.permission.RECORD_AUDIO == permission) {
				showToast(Toast.LENGTH_SHORT, com.serenegiant.dialog.R.string.permission_audio)
			}
			if (Manifest.permission.WRITE_EXTERNAL_STORAGE == permission) {
				showToast(Toast.LENGTH_SHORT, com.serenegiant.dialog.R.string.permission_ext_storage)
			}
			if (Manifest.permission.CAMERA == permission) {
				showToast(Toast.LENGTH_SHORT, com.serenegiant.dialog.R.string.permission_camera)
			}
			if (Manifest.permission.INTERNET == permission) {
				showToast(Toast.LENGTH_SHORT, com.serenegiant.dialog.R.string.permission_network)
			}
			if (Manifest.permission.ACCESS_FINE_LOCATION == permission) {
				showToast(Toast.LENGTH_SHORT, com.serenegiant.dialog.R.string.permission_location)
			}
		}
	}
			...
			if (dialog == null) {
				...
				if (Manifest.permission.INTERNET == permission) {
					RationalDialogV4.showDialog(this@MainActivity,
						com.serenegiant.dialog.R.string.permission_title,
						com.serenegiant.dialog.R.string.permission_network_request,
						arrayOf(Manifest.permission.INTERNET))
				} else if ((Manifest.permission.ACCESS_FINE_LOCATION == permission)
					|| (Manifest.permission.ACCESS_COARSE_LOCATION == permission)) {
					RationalDialogV4.showDialog(this@MainActivity,
						com.serenegiant.dialog.R.string.permission_title,
						com.serenegiant.dialog.R.string.permission_location_request,
