 ...
 *
 * modified 2022 by t_saki t_saki@serenegiant.com
 * 	to be enable to use external progress view
 *
...
public class SwipeRefreshLayout extends ViewGroup
	...
	public static final int LARGE = 0; // CircularProgressDrawable.LARGE;
	// Maps to ProgressBar default style = Maps to ProgressBar default style
	public static final int DEFAULT = 1; // CircularProgressDrawable.DEFAULT;
	...
	private boolean mOwnProgressView;
	@Nullable
...
//        mCircleView.setImageDrawable(mProgress);
	...
	public SwipeRefreshLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
		...
		if (DEBUG) Log.v(LOG_TAG, "コンストラクタ:");
		...
		final TypedArray attribs = context.getTheme().obtainStyledAttributes(
			attrs, R.styleable.SwipeRefreshLayout, 0, 0);
		try {
			mProgressViewId = attribs.getResourceId(R.styleable.SwipeRefreshLayout_progressView, View.NO_ID);
		} catch (final Exception e) {
			if (DEBUG) Log.w(LOG_TAG, e);
		}
		attribs.recycle();
...
//      moveToStart(1.0f);    // onAttachedToWindowへ移動
	...
	protected void onAttachedToWindow() {
		...
		if (mCircleView == null) {
			...
			View progressView = ViewUtils.findViewInParent(this, ids, View.class);
			if (progressView == null) {
				if (DEBUG) Log.v(LOG_TAG, "onAttachedToWindow:progress viewが指定されていないので内部生成する");
				mOwnProgressView = true;
				progressView = new AnimationProgressBar(getContext());
...
//    }
	...
	public void setProgressView(@NonNull final View progressView) {
		if (DEBUG) Log.v(LOG_TAG, "setProgressView:" + progressView);
		if (mCircleView != progressView) {
			mCircleView = progressView;
			updateProgressView();
		}
	}
...
//		mCircleView.setAnimationListener(listener);
	...
	private void startProgressAlphaStartAnimation() {
		mAlphaStartAnimation = startAlphaAnimation(getProgressAlpha()/*mProgress.getAlpha()*/, STARTING_PROGRESS_ALPHA);
	}

	private void startProgressAlphaMaxAnimation() {
		mAlphaMaxAnimation = startAlphaAnimation(getProgressAlpha()/*mProgress.getAlpha()*/, MAX_ALPHA);
	}
...
//                        (int) (startingAlpha + ((endingAlpha - startingAlpha) * interpolatedTime)));
		...
		if (mCircleView instanceof IAnimationView) {
			((IAnimationView) mCircleView).setAnimationListener(null);
		}
...
//        mProgress.setColorSchemeColors(colors);
	...
	@Override
	public boolean onStartNestedScroll(@NonNull final View child, @NonNull final View target, int axes, int type) {
		if (type == ViewCompat.TYPE_TOUCH) {
	...
	@Override
	public void onNestedScrollAccepted(@NonNull final View child, @NonNull final View target, int axes, int type) {
	...
	@Override
	public void onNestedScroll(@NonNull final View target, int dxConsumed, int dyConsumed, int dxUnconsumed,
	...
	public void onNestedPreScroll(
		...
		@NonNull final int[] consumed, int type) {
		// Should always be true because onStartNestedScroll returns false for all type !=
		// ViewCompat.TYPE_TOUCH, but check just in case.
		if (type == ViewCompat.TYPE_TOUCH) {
	...
	public boolean onStartNestedScroll(
		@NonNull final View child, @NonNull final View target,
...
//        mProgress.setArrowEnabled(true);
		...
		if (overscrollTop < mTotalDragDistance) {
			if (getProgressAlpha() > STARTING_PROGRESS_ALPHA
...
//            }
			...
			if (getProgressAlpha() < MAX_ALPHA && !isAnimationRunning(mAlphaMaxAnimation)) {
				// Animate the alpha
...
//            mProgress.setArrowScale(1 - interpolatedTime);
	...
	void moveToStart(float interpolatedTime) {
		...
		int offset = targetTop - (mCircleView != null ? mCircleView.getTop() : 0);
		setTargetOffsetTopAndBottom(offset);
	}
